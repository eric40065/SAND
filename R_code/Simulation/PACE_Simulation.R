...
source("./R_code/Simulation/get_all_data.R")
...
do_analysis = function(data_name_list = c("LowDim_G", "LowDim_T", "LowDim_E", "HighDim_G", "HighDim_T", "HighDim_E"), iidt_list = c("IID", "NonIID"),
                       ...
                       split = c(90, 5, 5), do_PACE = TRUE, do_1DS = TRUE, do_MICE = TRUE, do_trans = TRUE, save_plot = FALSE,
    ...
    for(iidt in iidt_list){
        ...
        for(data_name in data_name_list){
            ...
            for(dense_error in dense_error_list){# aaa}}}
                # dense_error = dense_error_list[[4]]
                print(iidt)
                print(data_name)
                print(dense_error)
...
# run_analysis = FALSE
if(run_analysis){
    result = do_analysis(do_PACE = FALSE, do_1DS = FALSE, do_MICE = FALSE, do_trans = TRUE, fast = FALSE, denoise_method_list = c("l2o"), iidt_list = "IID")
    # save result
    saveRDS(result, "./R_result/result.rds")
}else{
    # read 
    setwd("/Users/eric/Desktop/UCD/TransFD/")
    result = readRDS("./R_result/result.rds")
}
...
dev.off()
design = get_design_plot(time_list = result$fit_object$NonIID$LowDim_G$dense_w_error$fitPACE$fit$inputData$Lt)
