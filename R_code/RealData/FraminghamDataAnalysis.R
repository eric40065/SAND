...
get_design_plot = function(all_data){
    ...
    time = all_data$T_obs
    time_len = length(sort(unique(as.vector(time))))
...
myPACE = function(do_PACE, all_data, index, optns = NULL, Transformer = FALSE){
    ...
    if(do_PACE){
        ...
        ## from here
        MSE_train = mean((PACE_est[index$training, ] - all_data$X_obs[[index$training]])^2)
...
get_data_from_full = function(sparsity = c(8, 12), iidt = TRUE){
    ...
    X_full_noise = as.matrix(data.table::fread(file = "./Data/IID/RealData/X_full_noise.csv", header = FALSE))
    T_full = as.matrix(data.table::fread(file = "./Data/IID/RealData/T_full.csv", header = FALSE))
    ...
    folder = ifelse(iidt, "./Data/IID/RealData", "./Data/NonIID/RealData")
...
# generate data
all_data_list = list()
X_obs = X_full_noise = as.matrix(data.table::fread(file = "./Data/IID/RealData/Framingham/X_obs.csv", header = FALSE))
T_obs = X_full_noise = as.matrix(data.table::fread(file = "./Data/IID/RealData/Framingham/T_obs.csv", header = FALSE))
all_data_list[[1]] = list(X_obs = X_obs, T_obs = T_obs)
dev.off()
