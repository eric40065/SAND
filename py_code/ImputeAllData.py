...
if output_structure == "Vanilla":
    ...
    denoise_method_list = ["l1w", "l2w"]
    # denoise_method_list = ["TVo"]
elif output_structure == "SelfAtt":
    denoise_method_list = ["l2w"]
elif output_structure == "DiffSelfAtt":
    denoise_method_list = ["l2w"]
    ...
    for data_is_dense in list((True, False)):
        ...
        for denoise_method in denoise_method_list:
            ...
            for i in range(int(d/4)):
                d_T[0, 4*i+1, :] = np.sin(10 ** (2 - 4*(i + 1)/d) * t_true)
                d_T[0, 4*i+2, :] = np.cos(10 ** (2 - 4*(i + 1)/d) * t_true)
            ...
            prob_list = torch.softmax((1 - torch.abs(torch.Tensor(range(-L, L + 1))/L)) * 100, 0)
            prob_list[prob_list < 1e-15] = 0
            ...
            for counter, i in enumerate(index_all):
                ...
                e_m_mh = torch.repeat_interleave(e_m_mh, num_heads, dim = 0)
            ...
            for i, index in enumerate(index_all):
                ...
                if (i + 1) % 100 == 0 and index > 9450:
                    ...
                    plt.plot(t_true, org_pred[i, :, 0], label = "Original");
                    plt.fill_between(t_true, org_pred[i, :, 1], org_pred[i, :, 2], alpha=0.2, color = "blue");
                    if output_structure != "Vanilla":
                        plt.plot(t_true, smooth_pred[i, :, 0], label = "Smoothed");
                        plt.fill_between(t_true, smooth_pred[i, :, 1], smooth_pred[i, :, 2], alpha=0.2, color = "orange");
                    if data_name != "/Framingham":
                    ...
                    if output_structure == "Vanilla":
                        ...
                        plt.ylabel(output_structure, fontsize = 32)
...
np.round(error_mat_test_org.transpose() * 1000, 1)
np.round(error_mat_test_smooth.transpose() * 1000, 1)
