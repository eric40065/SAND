...
from ModelRelated import FeedForward
...
class MultiHeadAttention_SAD(nn.Module):
    """Multi-head attention."""
    def __init__(self, model_settings, output_structure = None):
        super(MultiHeadAttention, self).__init__()
        # output_structure = "SelfAtt" # "SelfAtt" None
        ...
        if self.num_heads > 1:
            self.head_NN = nn.Linear(self.num_heads, 1, bias = True)
        self.drop = nn.Dropout(dropout)
    ...
    def forward(self, q, k, v, mask = None):
        ...
        if self.output_structure != "SelfAtt":
            output = self.out(output)
        return self.head_NN(transpose_output(output, self.num_heads)) if self.num_heads > 1 else transpose_output(output, self.num_heads)
        # output size: (batch * num_heads, seq_len, num_hidden / num_heads)
